...
"use strict";
    ...
    $('#original').on('click', function(){
        ...
        let onloadInterval = localStorage.getItem('onloadInterval');
        if(onloadInterval){
            customizeChartData(onloadInterval);
        } else {
            customizeChartData(1440);
        }
    ...
    $('body').on('click', '.buy_qty', function() {
        ...
        feetxt = parseFloat(feetxt).toFixed(8); 
        feetxt2 = parseFloat(feetxt2).toFixed(8); 

        $("#sellfees").text(feetxt2.toString()+' '+market_details.market_symbol + ' (' + BDTASK.sellfees() + '%)');
        $('#sellfeesval').val(feetxt.toString());
    ...
    $("#buypricing").on("change", function(event) {
        ...
        $('#buywithout_feesval').val(parseFloat(buywithout_feesval).toString());
        var feetxt = (BDTASK.buyfees() / 100) * (buywithout_feesval);
        feetxt = parseFloat(feetxt).toFixed(8); //feetxt.toFixed(8);
        var fees = $("#buyfees").text(feetxt.toString() + market_details.market_symbol + ' (' + BDTASK.buyfees() + '%)');
        $('#buyfeesval').val(parseFloat(feetxt).toString());
        var total = +buywithout_feesval + +feetxt;
        $("#buytotal").text(parseFloat(total.toFixed(8)).toString());
        $('#buytotalval').val(parseFloat(total.toFixed(8)).toString());
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == 0) {
                    ...
                    $("#buytrades").prepend("<tr><td class='buy_price coin positive'>" + trade.trades.bid_price + "</td><td class='price'>" + trade.trades.bid_qty + "</td><td   data-id='"+trade.trades.id+"' class='change'>" + trade.trades.bid_qty + "</td></tr>");
                     chartDataUpdate();
                }
                ...
                $.getJSON(BDTASK.getSiteAction('get-open-order'), function(data) {
                   if(data.open_trade != null) {
                        $("#open_orders_table > tbody"). empty();
                        $.each(data.open_trade, function(index, trades) {   
                         var goUrl = '#'                     
                            $("#open_orders_table").prepend("<tr class='odd' width='100%'  id='tr_'"+trades.id+"'><td class='dtr-control sorting_1'>" + trades.bid_type + "</td><td>" + trades.bid_price + "</td><td  data-id='"+trades.id+"'>" + trades.bid_qty + "</td><td>" + trades.bid_qty_available + "</td><td>" + trades.total_amount + "</td><td>" + trades.amount_available + "</td><td>" + trades.market_symbol + "</td><td>" + trades.open_order + "</td><td  width='30'><div class='progress'><div title='Running' class='progress-bar progress-bar-striped width100percent'>Running</div></div></td><td><a data-id='"+ trades.id+"' href='" +goUrl+"' class='bg-danger text-white text-center cancel_trade' data-toggle='tooltip' data-placement='left' title='Cancel'>Cancel</a><td></tr>"); 
                        });
                    } else {
                        $("#open_orders_table > tbody"). empty();
                    }
    ...
    $("#sellpricing").on("change", function(event) {
        ...
        var feetxt2 = (BDTASK.sellfees() / 100) * sellwithout_fees;
        feetxt = parseFloat(feetxt).toFixed(8); //feetxt.toFixed(8);
        feetxt2 =parseFloat(feetxt2).toFixed(8); // feetxt2.toFixed(8);
        $("#sellfees").text(feetxt2.toString()+' '+ market_details.market_symbol + ' (' + BDTASK.sellfees() + '%)');
        $('#sellfeesval').val(parseFloat(feetxt).toString());
    ...
    $('body').on('click','.fiveMinute', function(response){
        ...
        $.each(userSetInterval, (index, value) => {
          if (value.v == dataValue) {
            a.push({ t: value.t, s: "1", v:value.v });
            localStorage.setItem('onloadInterval', dataValue);
            htmlData +=
              '<div data-value="'+value.v+'" class="fiveMinute mx-1 time-interval text-warning">' +
              value.t +
              '</div>';
          } else {
            a.push({ t: value.t, s: "0" , v:value.v});
            htmlData +=
              '<div data-value="'+value.v+'" class="fiveMinute mx-1 time-interval">' +
              value.t +
              '</div>';
          }
        });
        ...
        if(actionValue == "Edit"){
            customizeChartData(dataValue);
            $('#collapseButton').trigger('click');
        }
    });


    function customizeChartData(time = ""){

        $.getJSON(BDTASK.getSiteAction('timewise-chart-data?market='+market+"&time="+time), function(response) {

           // console.log('customize date wie chart data========', response);
          updatechartdata.updateSeries([{
            name: 'Market',
            data: response
          }])
...
$(function() {
    ...
    $('.cancel_trade').on('click', function(e){
    e.preventDefault();     
       var cid = $(this).attr('data-id');
       $.ajax({
            url: BDTASK.getSiteAction('order-cancel-ajax/'+cid),
            type: "GET",
            data: '',
            dataType: "JSON",
            success: function(data) {
                ...
                if(data.deleted == "ok"){                    
                  $('#open_orders_table tr#tr_'+ cid).remove();
              }
            }
        });      
    })

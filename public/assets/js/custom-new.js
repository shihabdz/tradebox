...
$(document).ready(function(){
	...
	$("#add_type").on("change", function(event) {
        ...
        $.getJSON(BDTASK.getSiteAction('backend/cms/getAdvertisementinfo/'+obj_id), function(data){
    ...
    $.getJSON(BDTASK.getSiteAction('backend/exchange/market_streamer'), function(data) {
    ...
    if($('#ajaxcointable').length){
        ...
        table = $('#ajaxcointable').DataTable({ 
            ...
            "ajax": {
                "url": BDTASK.getSiteAction('backend/exchange/cryptocoin-ajax-list'),
                "type": "POST",
                "data": inputdata
            },
   ...
   //Delete coin
    $('#ajaxcointable').on('click', '.actionDelete', function(e){
            e.preventDefault(); 
            var url = $(this).attr('data-url');
            var message = $(this).attr('data-message');           
            swal({
                title: "You are about to delete this coin. ",
                text: "Do you wish to delete this coin?",
                type: "warning",
              ...
              function(isConfirm) {
                if (isConfirm) {               
                 $.ajax({
                    ...
                    dataType: 'JSON',
                     error: function() {
                         swal("Coin not deleted!", "Something is wrong. Please try again", "error");
                     },
                     success: function(data) {
                        if(data.success==true){
                            swal("Deleted!", data.message, "success");
                            $('#ajaxcointable').DataTable().ajax.reload(null, false);
                        }else{
                             swal("Coin not deleted!", data.message, "error");                            
                        }                          
                     }
                  });
                } else {
                  swal("Cancelled", "Coin is safe :)", "error");
                }
              });
    ...
    $("#gatewayname").on("change", function(event) {
        ...
        $.getJSON(BDTASK.getSiteAction('backend/setting/getemailsmsgateway'), function(sms){
            ...
            if (gatewayname==='budgetsms') {
               ...
               $( "#sms_field").html("<div class='form-group row'><label for='api' class='col-sm-3 col-form-label'>"+display['apikey'][language]+"<i class='text-danger'>*</i></label><div class='col-sm-9'><input name='api' type='text' class='form-control' id='api' placeholder='"+display['apikey'][language]+"' value='"+api+"' required></div></div><div class='form-group row'><label for='password' class='col-sm-3 col-form-label'>"+display['app_secret'][language]+" <i class='text-danger'>*</i></label><div class='col-sm-9'><input name='password' type='password' class='form-control' id='password' placeholder='"+display['password'][language]+"' value='"+password+"' required></div></div>");
                ...
                $( "#sms_field").html("<div class='form-group row'><label for='sid' class='col-sm-3 col-form-label'>"+display['sid'][language]+"<i class='text-danger'>*</i></label><div class='col-sm-9'><input name='sid' type='text' class='form-control' id='sid' placeholder='"+display['sid'][language]+"' value='"+ sid +"' required></div></div><div class='form-group row'><label for='auth_token' class='col-sm-3 col-form-label'>"+display['auth_token'][language]+" <i class='text-danger'>*</i></label><div class='col-sm-9'><input name='auth_token' type='password' class='form-control' id='auth_token' placeholder='"+display['auth_token'][language]+"' value='"+token+"' required></div></div><div class='form-group row'><label for='number' class='col-sm-3 col-form-label'>"+display['phone_number'][language]+" <i class='text-danger'>*</i></label><div class='col-sm-9'><input name='phone_number' type='text' class='form-control' id='phone_number' placeholder='"+display['phone_number'][language]+"' value='"+number+"' required></div></div>");
    ...
    if($("#coin_value_USD").length){
        $.getJSON(BDTASK.getSiteAction("backend/total-referral-value"), function(data) {
            var sum_balance = 0;
        ...
        $.getJSON(BDTASK.getSiteAction("backend/all-fees-value"), function(data) {
...
function load_country_data(page, user) {
    if(user != ""){
        $.ajax({
            url: BDTASK.getSiteAction("backend/user/ajax-tradelist/"+user+"/"+page),
            method:"GET",
            dataType:"json",
            success:function(data)
            {
                console.log("sdfsdfdsfsdfdsf");
                console.log(data.country_table);
...
function jquery_ajax(id) {
   ...
   $.ajax({
        url : BDTASK.getSiteAction('backend/finance/user-info-load'),
        type: "POST",
        data: inputdata,
        dataType: "JSON",
        success: function(data)
...
function cancel_upnotification(t) {
    ...
    $.ajax({
        type: "POST",
        url: BDTASK.getSiteAction('backend/autoupdate/cancel_notification'),
        data: inputdata,
        success: function (t) {
            if(t == 1){

<?php namespace App\Modules\Website\Controllers;
	class PaymentCallbackController extends BaseController
	{
		...
		public function bitcoin_confirm($orderid = null){
			...
			$deposit      	= $this->db->table('crypto_payments')->select('*')->where('orderID', $orderid)->get()->row();
		...
		public function conipayment_withdraw()
		{
			...
			if ($status >= 100 || $status == 2) {
				...
				$dbt_withdraw_check = $this->db->select('*')->from("dbt_withdraw")->where("user_id",@$instantdata->user_id)->where("comment",$this->request->getPost("id", FILTER_SANITIZE_STRING))->where("status",1)->get()->getRow();

				if(empty(@$dbt_withdraw_check)){
					...
					$this->common_model->update('dbt_withdraw', $data, $wheredata);

					$t_data     = $this->db->table('dbt_withdraw')->select('*')->where($wheredata)->get()->getRow();
					...
					if($set->withdraw != NULL){
						...
						$check_user_balance = $this->db->table('dbt_balance')->select('*')->where('user_id', $user_id)->where('currency_symbol', $this->request->getPost('currency'))->get()->getRow();
						$new_balance = $check_user_balance->balance-$withdrawamount;

						$this->common_model->update('dbt_balance', array('balance' => $new_balance), array('user_id' => $user_id, 'currency_symbol0' => $this->request->getPost('currency')));

            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
               <div class="border_preview">
                  ...
                  <div class="form-group row">
                     ...
                     <div class="col-sm-8">
                        <input name="full_name" value="<?php echo esc($coinpair->full_name) ?>" class="form-control" placeholder="<?php echo display('full-name');?>" type="text" id="full_name">
                     </div>
                  </div>
                  <div class="form-group row">
                     ...
                     <div class="col-sm-8">
                        <select class="form-control basic-single" name="market_id" id="market_id">
                           ...
                           <option value="<?php echo $value->symbol; ?>" <?php echo ($coinpair->market_symbol==$value->symbol)?'Selected':'' ?>><?php echo esc($value->name); ?></option>
                           <?php } ?>
                        </select>
                     </div>
                  </div>
                  <div class="form-group row">
                     <label for="coin_id" class="col-sm-2 col-form-label font-weight-600"><?php echo display('coin') ?><i class="text-danger">*</i></label>
                     <div class="col-sm-8">
                        <select class="form-control basic-single" name="coin_id" id="coin_id">
                           <option value=""><?php echo display('select_option') ?></option>
                           <?php foreach ($coins as $key => $value) { ?>
                           <option value="<?php echo esc($value->symbol); ?>" <?php echo ($coinpair->currency_symbol==$value->symbol)?'Selected':'' ?>><?php echo esc($value->symbol); ?></option>
                           <?php } ?>
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="form-group row">
                     <label for="status" class="col-sm-2 col-form-label font-weight-600"><?php echo display('default_pair'). display('pair') ?></label>
                     <div class="col-sm-8 mb-4 pt10">
                        <label class="radio-inline">
                        <?php echo form_radio('isDefault', '1', (($coinpair->isDefault==1)?true:false)); ?><?php echo display('yes');?>
                        </label>
                        <label class="radio-inline">
                        <?php echo form_radio('isDefault', '0', (($coinpair->isDefault==0)?true:false) ); ?><?php echo display('no');?>
                        </label> 
                     </div>
                  </div>
                  <div class="row">
